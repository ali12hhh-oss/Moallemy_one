...
class LearningRecommendationV9 {
  ...
  const LearningRecommendationV9({
    required this.type,
    required this.title,
...
class AdaptiveLearningEngineV9 {
  ...
  static Future<void> recordAnswer({
    ...
    final old = SkillProgressV9.fromMap(
      skill,
      Map<String, dynamic>.from(skills[skill] ?? {}),
    );
    final next = SkillProgressV9(
      skillId: skill,
  ...
  static Future<Map<String, SkillProgressV9>> skills(String stageId) async {
    ...
    final stage = Map<String, dynamic>.from(
      Map<String, dynamic>.from(data['stages'] ?? {})[stageId] ?? {},
    );
    final raw = Map<String, dynamic>.from(stage['skills'] ?? {});
    return raw.map(
      (k, v) =>
          MapEntry(k, SkillProgressV9.fromMap(k, Map<String, dynamic>.from(v))),
    );
  }
  ...
  static Future<bool> isStageReady(
  ...
  static Future<List<LearningRecommendationV9>> recommendations(
    CurriculumStageV8 stage,
    Set<String> completedUnits,
  ) async {
    ...
    for (final skillId in weak.take(3)) {
      out.add(
        LearningRecommendationV9(
          type: 'review',
          title: 'مراجعة $skillId',
          reason: 'هذه المهارة تحتاج تدريباً إضافياً',
          icon: '🔄',
          skill: skillId,
        ),
      );
    ...
    if (out.isEmpty)
      out.add(
        const LearningRecommendationV9(
          type: 'mastery',
          title: 'أنت جاهز للاختبار النهائي',
          reason: 'أتقنت دروس المرحلة والمهارات المسجلة',
          icon: '🏆',
        ),
      );
    return out;
  ...
  static Future<void> recordUnitPractice(
    String stageId,
    String unitId,
    String skill,
  ) async {
