...
class ArabicGrammarScreenV12 extends StatefulWidget {
  ...
  @override
  State<ArabicGrammarScreenV12> createState() => _ArabicGrammarScreenV12State();
}
...
class _ArabicGrammarScreenV12State extends State<ArabicGrammarScreenV12> {
  ...
  List<Widget> _practiceFor(ArabicGrammarLessonV12 x) {
    ...
    if (data == null)
      return x.exercises
          .map(
            (e) =>
                ListTile(leading: const Icon(Icons.edit_note), title: Text(e)),
          )
          .toList();
    return [
      Padding(
  ...
  void _answer(_Question q, String answer) {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text(correct ? 'أحسنت! إجابة صحيحة ⭐' : 'حاول مرة أخرى 🌱'),
      ),
    );
