...
const _examColors = <(String, Color)>[
  ...
  ('أخضر', Color(0xFF43A047)), ('برتقالي', Color(0xFFFB8C00)), ('بنفسجي', Color(0xFF8E24AA)),
  ('أبيض', Color(0xFFECEFF1)), ('أسود', Color(0xFF212121)),
];

enum _ShapeKind { square, triangle, circle, rectangle, trapezoid, oblique }

const _examShapes = <(String, _ShapeKind)>[
  ('مربع', _ShapeKind.square), ('مثلث', _ShapeKind.triangle), ('دائرة', _ShapeKind.circle),
  ('مستطيل', _ShapeKind.rectangle), ('منحرف', _ShapeKind.oblique), ('شبه منحرف', _ShapeKind.trapezoid),
];

const _examPairs = <(String, String)>[
  ('د', 'ا'), ('ن', 'ا'), ('ب', 'ا'), ('م', 'ا'), ('س', 'ا'), ('ل', 'ا'), ('ف', 'ي'), ('ه', 'ي'),
];
...
List<_ExamQ> _buildBank(Random rnd) {
  ...
  for (var k = 0; k < 2; k++) {
    ...
    bank.add(_ExamQ(
      ...
      visual: Container(width: 90, height: 90, decoration: BoxDecoration(color: target.$2, shape: BoxShape.circle, border: Border.all(color: Colors.black12, width: 2))),
      options: opts,
  ...
  for (var k = 0; k < 2; k++) {
    ...
    bank.add(_ExamQ(
      ...
      visual: SizedBox(width: 90, height: 90, child: CustomPaint(painter: _ExamShapePainter(target.$2))),
      options: opts,
      correct: opts.indexOf(target.$1),
    ));
  }
  ...
  for (var k = 0; k < 2; k++) {
    ...
    bank.add(_ExamQ(
      ...
      visual: Text('${target.$1} + ${target.$2}', style: const TextStyle(fontSize: 44, fontWeight: FontWeight.w900)),
      options: opts,
  ...
  for (var k = 0; k < 2; k++) {
    ...
    bank.add(_ExamQ(
      category: 'آحاد',
      skillKey: 'exam.ones',
      visual: Wrap(alignment: WrapAlignment.center, spacing: 6, children: List.generate(n, (_) => Text(icon, style: const TextStyle(fontSize: 26)))),
      options: opts.map(arNum).toList(),
      correct: opts.indexOf(n),
    ));
  }
    ...
    bank.add(_ExamQ(
      ...
      visual: Wrap(
        ...
        children: List.generate(groups, (_) => Container(
          ...
          child: Wrap(spacing: 2, runSpacing: 2, children: List.generate(10, (_) => Container(width: 6, height: 6, decoration: const BoxDecoration(color: Color(0xFF00C853), shape: BoxShape.circle)))),
        )),
      ),
      options: opts.map(arNum).toList(),
      correct: opts.indexOf(n),
...
class _PrepExamScreenState extends State<PrepExamScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Directionality(
      ...
      child: Scaffold(
        appBar: AppBar(title: Text('اختبار التمهيدي • ${index + 1} / ${questions.length}')),
        body: Stack(children: [
          Padding(
            ...
            child: Column(children: [
              ...
              Chip(label: Text(q.category, style: const TextStyle(fontWeight: FontWeight.bold))),
              const SizedBox(height: 14),
              if (q.speak != null) ...[
                IconButton(iconSize: 54, icon: const Icon(Icons.volume_up_rounded), onPressed: q.speak),
                const Text('استمع واختر الإجابة الصحيحة', style: TextStyle(fontSize: 14)),
              ] else
                q.visual,
                ...
                child: GridView.count(
                  ...
                  children: List.generate(q.options.length, (i) {
                    return Button3D(
                      ...
                      child: Center(child: Text(q.options[i], style: const TextStyle(fontSize: 26, fontWeight: FontWeight.w900, color: Colors.white))),
                    );
                  }),
                ),
              ),
          ...
          child: Padding(
            ...
            child: Container(
              ...
              child: Column(mainAxisSize: MainAxisSize.min, children: [
                ...
                const SizedBox(height: 14),
                Text(passed ? 'مبروك! اجتزت اختبار التمهيدي' : 'أحسنت المحاولة!', style: const TextStyle(fontSize: 24, fontWeight: FontWeight.w900, color: Colors.white), textAlign: TextAlign.center),
                ...
                Button3D(
                  ...
                  child: Text('عودة', style: TextStyle(fontSize: 17, fontWeight: FontWeight.w900, color: passed ? const Color(0xFFFF7043) : const Color(0xFF1E88E5))),
                ),
              ]),
            ),
          ),
