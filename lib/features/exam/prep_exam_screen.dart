...
List<_ExamQ> _buildBank(Random rnd) {
  ...
  for (var k = 0; k < 4; k++) {
    ...
    bank.add(_ExamQ(
      ...
      visual: Text(target.letter, style: const TextStyle(fontSize: 72, fontWeight: FontWeight.w900)),
      options: opts,
  ...
  for (var k = 0; k < 4; k++) {
    ...
    final candidates = List.generate(10, (i) => i + 1)..remove(answer)..shuffle(rnd);
    ...
    bank.add(_ExamQ(
      ...
      visual: Text(prompt, textAlign: TextAlign.center, style: const TextStyle(fontSize: 24, fontWeight: FontWeight.w900)),
      options: optionsNumbers.map(arNum).toList(),
      correct: optionsNumbers.indexOf(answer),
      speak: mode == 0 ? () => VoiceService.arabic(_numberWords[n] ?? '$n') : null,
    ));
  }
  ...
  for (var k = 0; k < 3; k++) {
    ...
    bank.add(_ExamQ(
      ...
      visual: Container(width: 100, height: 100, decoration: BoxDecoration(color: target.$2, shape: BoxShape.circle, border: Border.all(color: Colors.black12, width: 2))),
      options: opts,
  ...
  for (var k = 0; k < 3; k++) {
    ...
    bank.add(_ExamQ(
      ...
      visual: SizedBox(width: 110, height: 110, child: CustomPaint(painter: _ExamShapePainter(target.$2))),
      options: opts,
      correct: opts.indexOf(target.$1),
      speak: k == 1 ? () => VoiceService.arabic(target.$1) : null,
    ));
  ...
  bank.add(_ExamQ(
    ...
    visual: Text(targetLetter.letter, style: const TextStyle(fontSize: 80, fontWeight: FontWeight.w900)),
    options: letterOpts,
    correct: letterOpts.indexOf(targetLetter.letter),
    speak: () => VoiceService.arabicLetterSound(targetLetter.letter, fallbackText: targetLetter.sound),
  ));
  final writtenNumber = 1 + rnd.nextInt(20);
  final numberOthers = List.generate(20, (i) => i + 1)..remove(writtenNumber)..shuffle(rnd);
  final writtenOpts = [writtenNumber, ...numberOthers.take(3)]..shuffle(rnd);
  bank.add(_ExamQ(
    category: 'الكتابة',
    skillKey: 'exam.writing',
    visual: Text(arNum(writtenNumber), style: const TextStyle(fontSize: 72, fontWeight: FontWeight.w900)),
  ...
  for (var k = 0; k < 4; k++) {
    ...
    bank.add(_ExamQ(
      ...
      visual: Text('${target.$1} + ${target.$2}', style: const TextStyle(fontSize: 46, fontWeight: FontWeight.w900)),
      options: opts,
  ...
  for (var k = 0; k < 4; k++) {
    ...
    final candidates = List.generate(10, (i) => i)..remove(answer)..shuffle(rnd);
...
class _PlaceValueBox extends StatelessWidget {
  ...
  const _PlaceValueBox({required this.label, required this.value, required this.active});
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        border: Border.all(color: active ? const Color(0xFF1A237E) : Colors.black12, width: 2),
      ),
      child: Column(
        children: [
          Text(label, style: TextStyle(color: active ? Colors.white : const Color(0xFF3949AB), fontWeight: FontWeight.w900)),
          const SizedBox(height: 6),
          Text(arNum(value), style: TextStyle(color: active ? Colors.white : const Color(0xFF3949AB), fontSize: 36, fontWeight: FontWeight.w900)),
        ],
      ),
    );
  }
...
class _ExamShapePainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    final paint = Paint()..color = const Color(0xFF7C4DFF)..style = PaintingStyle.fill;
    final w = size.width;
    final h = size.height;
    final path = Path();
    switch (kind) {
...
class _PrepExamScreenState extends State<PrepExamScreen> {
  ...
  Widget build(BuildContext context) {
    if (finished) {
      return _ResultScreen(score: correctFirstTry, total: questions.length, title: resultTitle, gift: resultGift, breakdown: _breakdown());
    }
    if (!started) return _IntroScreen(onStart: _start);
    return _ExamScreen(question: questions[index], index: index, total: questions.length, cheer: cheer, onAnswer: _answer);
  }
  ...
  Map<String, String> _breakdown() {
    ...
    for (final entry in totalByCategory.entries) {
      ...
      result[entry.key] = _levelText(correct / entry.value);
    }
    return result;
  }
        ...
        body: ListView(
          ...
          children: [
            Container(
              ...
              decoration: BoxDecoration(
                gradient: const LinearGradient(colors: [Color(0xFF7C4DFF), Color(0xFF00BFA5)]),
              ...
              child: const Column(
                children: [
                  ...
                  SizedBox(height: 8),
                  Text('لنكتشف مستواك!', textAlign: TextAlign.center, style: TextStyle(color: Colors.white, fontSize: 27, fontWeight: FontWeight.w900)),
                  SizedBox(height: 8),
                  Text('أسئلة قصيرة ومتنوعة من مهارات الروضة الأولى والثانية. أجب بهدوء واستمع للسؤال عندما يظهر زر السماعة.', textAlign: TextAlign.center, style: TextStyle(color: Colors.white, fontSize: 15)),
                ],
              ),
            ),
            const SizedBox(height: 18),
            ..._sectionInfo.map((section) => Padding(
              ...
              child: Container(
                ...
                child: Row(
                  children: [
                    CircleAvatar(backgroundColor: section.color, child: Icon(section.icon, color: Colors.white)),
            ...
            Button3D(
              ...
              child: const Row(mainAxisAlignment: MainAxisAlignment.center, children: [
                ...
                SizedBox(width: 8),
                Text('ابدأ تحديد المستوى', style: TextStyle(color: Colors.white, fontSize: 21, fontWeight: FontWeight.w900)),
              ]),
            ),
          ],
        ),
...
class _ExamScreen extends StatelessWidget {
  ...
  const _ExamScreen({required this.question, required this.index, required this.total, required this.cheer, required this.onAnswer});
  ...
  Widget build(BuildContext context) {
    return Directionality(
      textDirection: TextDirection.rtl,
      child: Scaffold(
        appBar: AppBar(title: Text('تحديد المستوى • ${arNum(index + 1)} / ${arNum(total)}')),
        body: Stack(
          children: [
            Padding(
              ...
              child: Column(
                children: [
                  LinearProgressIndicator(value: (index + 1) / total, minHeight: 8, borderRadius: BorderRadius.circular(8)),
                  const SizedBox(height: 14),
                  Container(
                    padding: const EdgeInsets.symmetric(horizontal: 14, vertical: 8),
                    decoration: BoxDecoration(color: const Color(0xFF7C4DFF).withValues(alpha: .12), borderRadius: BorderRadius.circular(16)),
                    child: Text(question.category, style: const TextStyle(fontWeight: FontWeight.w900)),
                  ),
                  const SizedBox(height: 16),
                  Expanded(child: Column(children: [
                    Expanded(child: Center(child: question.visual)),
                    if (question.speak != null) ...[
                      Button3D(
                        ...
                        child: const Row(mainAxisSize: MainAxisSize.min, children: [
                          ...
                          SizedBox(width: 8),
                          Text('استمع للسؤال', style: TextStyle(color: Colors.white, fontWeight: FontWeight.w900, fontSize: 16)),
                        ]),
                      ),
                    ...
                    SizedBox(
                      ...
                      child: GridView.count(
                        ...
                        mainAxisSpacing: 12,
                        crossAxisSpacing: 12,
                        children: List.generate(question.options.length, (i) => Button3D(
                          onTap: () => onAnswer(i),
                          color: const Color(0xFF7C4DFF),
                          child: Center(child: Text(question.options[i], textAlign: TextAlign.center, style: const TextStyle(fontSize: 24, fontWeight: FontWeight.w900, color: Colors.white))),
                        )),
                      ),
                    ),
                  ])),
...
class _ResultScreen extends StatelessWidget {
  ...
  const _ResultScreen({required this.score, required this.total, required this.title, required this.gift, required this.breakdown});
        ...
        body: ListView(
          ...
          children: [
            Container(
              ...
              decoration: BoxDecoration(
                gradient: LinearGradient(colors: passed ? const [Color(0xFFFFD54F), Color(0xFFFF7043)] : const [Color(0xFF90CAF9), Color(0xFF64B5F6)]),
                borderRadius: BorderRadius.circular(28),
              ),
              child: Column(children: [
                Text(passed ? '🏆🎉⭐' : '🌱💪', style: const TextStyle(fontSize: 50)),
                const SizedBox(height: 8),
                Text(passed ? 'أحسنت! اكتشفنا مستواك' : 'بداية جميلة! استمر', textAlign: TextAlign.center, style: const TextStyle(color: Colors.white, fontSize: 24, fontWeight: FontWeight.w900)),
                const SizedBox(height: 8),
                Text('النتيجة: ${arNum(score)} من ${arNum(total)}', style: const TextStyle(color: Colors.white, fontSize: 20, fontWeight: FontWeight.w900)),
                const SizedBox(height: 6),
                Text('المستوى المقترح: $level', style: const TextStyle(color: Colors.white, fontSize: 18, fontWeight: FontWeight.w900)),
              ]),
            ),
            const SizedBox(height: 16),
            Container(
              padding: const EdgeInsets.all(18),
              decoration: BoxDecoration(color: const Color(0xFFFFF3E0), borderRadius: BorderRadius.circular(22), border: Border.all(color: const Color(0xFFFFB74D))),
              child: Column(children: [
                ...
                const SizedBox(height: 8),
                Text(passed ? '⭐ حصلت على مكافأة الاختبار أيضًا' : '🌱 هذا اللقب بداية طريقك الجميل', textAlign: TextAlign.center),
              ]),
            ),
            const SizedBox(height: 16),
            const Text('تحليل المهارات', style: TextStyle(fontSize: 21, fontWeight: FontWeight.w900)),
            const SizedBox(height: 10),
            ...breakdown.entries.map((entry) => Card(
              margin: const EdgeInsets.only(bottom: 8),
              child: ListTile(
                leading: const Icon(Icons.check_circle_rounded),
                title: Text(entry.key, style: const TextStyle(fontWeight: FontWeight.w800)),
                trailing: Text(entry.value, style: const TextStyle(fontWeight: FontWeight.w900)),
              ),
            )),
            const SizedBox(height: 10),
            Button3D(
              ...
              child: const Text('عودة', textAlign: TextAlign.center, style: TextStyle(color: Colors.white, fontSize: 18, fontWeight: FontWeight.w900)),
            ),
          ],
        ),
      ),
