        ...
        body: Stack(children: [
          Padding(
            ...
            child: Column(children: [
              ...
              if (!split)
                Button3D(
                  onTap: () { VoiceService.arabic(w.word); _finishedReading(); },
                  color: color,
                ...
                Wrap(
                  ...
                  spacing: 12,
                  children: w.letters.map((l) => Button3D(
                    onTap: () => VoiceService.arabicLetterSound(l),
                    color: color,
              ...
              OutlinedButton.icon(
                onPressed: () { VoiceService.stop(); setState(() => split = !split); },
                icon: Icon(split ? Icons.merge_rounded : Icons.call_split_rounded),
                label: SpeakableText(split ? 'اجمع الحروف كلمة واحدة' : 'قسّم الكلمة إلى حروف'),
              ),
              const SizedBox(height: 8),
              SpeakableText('اضغط الكلمة أو الحروف لتسمعها', style: TextStyle(color: Theme.of(context).colorScheme.onSurfaceVariant)),
              const Spacer(),
              Row(children: [
                Expanded(child: OutlinedButton(onPressed: () => _next(-1), child: const SpeakableText('السابقة'))),
                const SizedBox(width: 10),
                Expanded(child: FilledButton(onPressed: () => _next(1), child: const SpeakableText('التالية'))),
              ]),
            ]),
          ),
