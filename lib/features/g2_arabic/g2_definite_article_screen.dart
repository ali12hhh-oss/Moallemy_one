...
class G2DefiniteArticleScreen extends StatefulWidget {
  ...
  @override
  State<G2DefiniteArticleScreen> createState() => _G2DefiniteArticleScreenState();
}
...
class _G2DefiniteArticleScreenState extends State<G2DefiniteArticleScreen> {
  ...
  void _nextQuiz() {
    ...
    final others = definiteArticlePairs.map((p) => p.defined).where((d) => d != target.defined).toList()..shuffle(rnd);
          ...
          Column(children: [
            ...
            if (learnMode) ...[
              const Padding(
                ...
                child: Text('عندما نضيف "ال" في بداية الكلمة، تتحوّل من كلمة عامة (نكرة) إلى كلمة محددة (معرفة).', textAlign: TextAlign.center),
              ),
              Expanded(
                child: ListView.builder(
                  ...
                  itemBuilder: (_, i) {
                    ...
                    return Padding(
                      ...
                      child: Button3D(
                        onTap: () => VoiceService.arabic('${p.bare}... ${p.defined}'),
                        color: const Color(0xFF7C4DFF),
                        padding: const EdgeInsets.symmetric(horizontal: 18, vertical: 16),
                        child: Row(mainAxisAlignment: MainAxisAlignment.center, children: [
                          Text(p.emoji, style: const TextStyle(fontSize: 26)),
                          const SizedBox(width: 10),
                          Text(p.bare, style: const TextStyle(fontSize: 22, color: Colors.white)),
                          const SizedBox(width: 10),
                          const Icon(Icons.arrow_back_rounded, color: Colors.white70, size: 18),
                          const SizedBox(width: 10),
                          Text(p.defined, style: const TextStyle(fontSize: 22, fontWeight: FontWeight.w900, color: Colors.white)),
                        ]),
                      ),
                    );
                  },
              ...
              Expanded(
                child: Padding(
                  ...
                  child: Column(children: [
                    Text('ما الشكل المعرَّف لكلمة:', style: TextStyle(color: Theme.of(context).colorScheme.onSurfaceVariant)),
                    const SizedBox(height: 8),
                    Text('${target.emoji}  ${target.bare}', style: const TextStyle(fontSize: 34, fontWeight: FontWeight.w900)),
                    ...
                    Expanded(
                      child: GridView.count(
                        ...
                        mainAxisSpacing: 14,
                        crossAxisSpacing: 14,
                        children: options.map((o) => Button3D(onTap: () => _answer(o), color: const Color(0xFF00C853), child: Center(child: Text(o, style: const TextStyle(fontSize: 24, fontWeight: FontWeight.w900, color: Colors.white))))).toList(),
                      ),
                    ),
                  ]),
                ),
