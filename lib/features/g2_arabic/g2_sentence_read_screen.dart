...
class G2SentenceReadScreen extends StatefulWidget {
  ...
  final String title;
  const G2SentenceReadScreen({super.key, this.sentences = twoPartSentences, this.title = 'جمل قصيرة'});
  @override
...
class _G2SentenceReadScreenState extends State<G2SentenceReadScreen> {
  ...
  void _next(int delta) {
    setState(() {
      index = (index + delta + widget.sentences.length) % widget.sentences.length;
      split = false;
    });
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Directionality(
      ...
      child: Scaffold(
        appBar: AppBar(title: Text('${widget.title} • ${index + 1} من ${widget.sentences.length}')),
        body: Stack(children: [
          Padding(
            padding: const EdgeInsets.all(20),
            child: Column(children: [
              LinearProgressIndicator(value: (index + 1) / widget.sentences.length),
              ...
              if (!split)
                Button3D(
                  ...
                  child: Text(s.sentence, style: const TextStyle(fontSize: 34, fontWeight: FontWeight.w900, color: Colors.white)),
                )
              else
                Wrap(
                  ...
                  children: s.parts.map((p) {
                    return Button3D(
                      ...
                      child: Text(p, style: const TextStyle(fontSize: 26, fontWeight: FontWeight.w900, color: Colors.white)),
                    );
                  }).toList(),
                ),
              ...
              OutlinedButton.icon(
                ...
                label: Text(split ? 'اجمع الجملة كاملة' : 'قسّم الجملة إلى كلمات'),
              ),
              const Spacer(),
              Row(children: [
