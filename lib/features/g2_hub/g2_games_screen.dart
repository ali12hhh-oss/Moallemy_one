...
class _MathGameState extends State<_MathGame> {
  final rnd = Random(); late String question; late int answer; late List<int> options; int score = 0; String? cheer;
  @override void initState() { super.initState(); _next(); }
...
class _GrammarGameState extends State<_GrammarGame> {
  final rnd = Random(); bool nounVerbRound = true; late NounVerbWord nvTarget; late GenderPair genderPair; late bool genderShowFeminine; int score = 0; String? cheer;
  @override void initState() { super.initState(); _next(); }
  void _next() { nounVerbRound = rnd.nextBool(); nvTarget = nounVerbWords[rnd.nextInt(nounVerbWords.length)]; genderPair = genderPairs[rnd.nextInt(genderPairs.length)]; genderShowFeminine = rnd.nextBool(); WidgetsBinding.instance.addPostFrameCallback((_) { VoiceService.arabic(nounVerbRound ? nvTarget.word : (genderShowFeminine ? genderPair.feminine : genderPair.masculine)); }); }
  void _answer(bool chosenSecondOption) { final isCorrect = nounVerbRound ? (chosenSecondOption == nvTarget.isVerb) : (chosenSecondOption == genderShowFeminine); if (isCorrect) { score++; setState(() => cheer = kCheers[rnd.nextInt(kCheers.length)]); ProgressV8.addRewards(stars: 1, xp: 5); Future.delayed(const Duration(seconds: 2), () { if (mounted) { setState(() => cheer = null); _next(); } }); } else { setState(() => cheer = 'حاول مرة أخرى 💪'); Future.delayed(const Duration(milliseconds: 900), () { if (mounted) setState(() => cheer = null); }); } setState(() {}); }
  @override Widget build(BuildContext context) { final word = nounVerbRound ? nvTarget.word : (genderShowFeminine ? genderPair.feminine : genderPair.masculine); final emoji = nounVerbRound ? nvTarget.emoji : (genderShowFeminine ? genderPair.emojiF : genderPair.emojiM); final leftLabel = nounVerbRound ? 'اسم' : 'مذكر'; final rightLabel = nounVerbRound ? 'فعل' : 'مؤنث'; return Directionality(textDirection: TextDirection.rtl, child: Scaffold(appBar: AppBar(title: Text('لعبة القواعد • ${arNum(score)} ⭐')), body: Stack(children: [Padding(padding: const EdgeInsets.all(20), child: Column(children: [Text(emoji, style: const TextStyle(fontSize: 60)), Text(word, style: const TextStyle(fontSize: 34, fontWeight: FontWeight.w900)), const SizedBox(height: 30), Row(children: [Expanded(child: Button3D(onTap: () => _answer(false), color: const Color(0xFF2979FF), padding: const EdgeInsets.symmetric(vertical: 22), child: Center(child: Text(leftLabel, style: const TextStyle(fontSize: 19, fontWeight: FontWeight.w900, color: Colors.white))))), const SizedBox(width: 14), Expanded(child: Button3D(onTap: () => _answer(true), color: const Color(0xFFFF1E7E), padding: const EdgeInsets.symmetric(vertical: 22), child: Center(child: Text(rightLabel, style: const TextStyle(fontSize: 19, fontWeight: FontWeight.w900, color: Colors.white)))))]), ])), CelebrationOverlay(message: cheer)])); }
}
