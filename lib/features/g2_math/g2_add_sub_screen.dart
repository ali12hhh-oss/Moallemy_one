...
/// جمع/طرح بأرقام حتى ٩٩، بصيغتين: أفقية وعمودية، لتعليم الطفل الطريقتين.
class G2AddSubScreen extends StatefulWidget {
  final bool isAddition;
  const G2AddSubScreen({super.key, required this.isAddition});

  @override
  State<G2AddSubScreen> createState() => _G2AddSubScreenState();
}

class _G2AddSubScreenState extends State<G2AddSubScreen> {
  final rnd = Random();
  bool vertical = false;
  late int a, b, answer;
  ...
  void _next() {
    if (widget.isAddition) {
      a = 10 + rnd.nextInt(70);
      b = 1 + rnd.nextInt(99 - a > 20 ? 20 : max(1, 99 - a));
      answer = a + b;
    } else {
      a = 20 + rnd.nextInt(79);
