...
class _G3QuestionWordsScreenState extends State<G3QuestionWordsScreen> {
  ...
  void _nextQuiz() {
    ...
    final others = questionWords.map((q) => q.word).where((w) => w != target.word).toList()..shuffle(rnd);
                  ...
                  itemBuilder: (_, i) {
                    ...
                    return Padding(
                      ...
                      child: Button3D(
                        ...
                        child: Row(children: [
                          Text(q.emoji, style: const TextStyle(fontSize: 26)),
                          const SizedBox(width: 10),
                          Expanded(
                            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                              Text(q.word, style: const TextStyle(fontSize: 20, fontWeight: FontWeight.w900, color: Colors.white)),
                              Text(q.example, style: const TextStyle(color: Colors.white70, fontSize: 13)),
                              Text(q.usage, style: const TextStyle(color: Colors.white60, fontSize: 11)),
                            ]),
                          ),
                        ]),
                      ),
              ...
              Expanded(
                child: Padding(
                  ...
                  child: Column(children: [
                    const Text('أي أداة استفهام نستخدمها هنا؟', style: TextStyle(fontSize: 17, fontWeight: FontWeight.bold)),
                    const SizedBox(height: 10),
                    Text(target.usage, style: const TextStyle(fontSize: 20), textAlign: TextAlign.center),
                    ...
                    Expanded(
                      child: GridView.count(
                        ...
                        mainAxisSpacing: 14,
                        crossAxisSpacing: 14,
                        children: options.map((o) => Button3D(onTap: () => _answer(o), color: const Color(0xFF2979FF), child: Center(child: Text(o, style: const TextStyle(fontSize: 24, fontWeight: FontWeight.w900, color: Colors.white))))).toList(),
                      ),
                    ),
                  ]),
                ),
