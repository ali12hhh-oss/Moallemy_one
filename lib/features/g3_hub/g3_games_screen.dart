...
class _EnglishGameState extends State<_EnglishGame> {
  ...
  void _next() {
    ...
    WidgetsBinding.instance.addPostFrameCallback(
      (_) => VoiceService.english(target.word),
    );
  }
              ...
              child: Column(
                children: [
                  IconButton(
                    ...
                    onPressed: () => VoiceService.english(target.word),
                  ),
                  const Text(
                    'ما معنى هذه الكلمة؟',
                    style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold),
...
class _MultiplicationGameState extends State<_MultiplicationGame> {
  ...
  void _next() {
    ...
    b = 1 + rnd.nextInt(10);
    answer = a * b;
    final others = {for (var i = max(1, answer - 8); i <= answer + 8; i++) i}
      ..remove(answer);
...
class _DivisionGameState extends State<_DivisionGame> {
  ...
  void _next() {
    ...
    final others = {for (var i = max(1, answer - 4); i <= answer + 4; i++) i}
      ..remove(answer);
    final list = others.toList()..shuffle(rnd);
    options = [answer, ...list.take(3)]..shuffle(rnd);
  }
