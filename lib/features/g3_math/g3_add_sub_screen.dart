...
class _G3AddSubScreenState extends State<G3AddSubScreen> {
  ...
  void _next() {
    ...
    final others = {for (var i = max(0, answer - 8); i <= answer + 8; i++) i}..remove(answer);
        ...
        body: Stack(children: [
          Padding(
            ...
            child: Column(children: [
              ...
              if (!vertical)
                Text('${arNum(a)} $op ${arNum(b)} = ؟', style: const TextStyle(fontSize: 30, fontWeight: FontWeight.w900))
              else
                SizedBox(
                  width: 160,
                  child: Column(crossAxisAlignment: CrossAxisAlignment.end, children: [
                    Text(arNum(a), style: const TextStyle(fontSize: 30, fontWeight: FontWeight.w900)),
                    Row(mainAxisAlignment: MainAxisAlignment.end, children: [
                      Text(op, style: const TextStyle(fontSize: 26)),
                      const SizedBox(width: 8),
                      Text(arNum(b), style: const TextStyle(fontSize: 30, fontWeight: FontWeight.w900)),
                    ]),
                    Container(height: 3, color: Colors.black87, margin: const EdgeInsets.symmetric(vertical: 4)),
                    const Text('؟', style: TextStyle(fontSize: 30, fontWeight: FontWeight.w900, color: Colors.grey)),
                  ]),
                ),
                ...
                child: GridView.count(
                  ...
                  children: options.map((o) {
                    return Button3D(
                      ...
                      child: Center(child: Text(arNum(o), style: const TextStyle(fontSize: 26, fontWeight: FontWeight.w900, color: Colors.white))),
                    );
                  }).toList(),
                ),
              ),
