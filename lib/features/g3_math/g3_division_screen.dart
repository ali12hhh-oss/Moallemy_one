...
class _G3DivisionScreenState extends State<G3DivisionScreen> {
  ...
  void _generate() {
    ...
    final others = {for (var i = max(1, quotient - 3); i <= quotient + 3; i++) i}..remove(quotient);
              ...
              child: Padding(
                ...
                child: Column(children: [
                  ...
                  Wrap(
                    ...
                    children: List.generate(divisor, (g) {
                      return Container(
                        ...
                        child: Wrap(
                          spacing: 2,
                          children: List.generate(learnMode ? quotient : 0, (_) => Text(icon, style: const TextStyle(fontSize: 22))),
                        ),
                      );
                    }),
                  ),
                  ...
                  Row(mainAxisAlignment: MainAxisAlignment.center, children: [
                    IconButton(icon: const Icon(Icons.volume_up_rounded), onPressed: () => VoiceService.arabic('${arNum(dividend)} تقسيم ${arNum(divisor)}')),
                    Text('${arNum(dividend)} ÷ ${arNum(divisor)} = ${learnMode ? arNum(quotient) : '؟'}', style: const TextStyle(fontSize: 26, fontWeight: FontWeight.w900)),
                  ]),
                  const SizedBox(height: 16),
                  if (learnMode)
                    FilledButton.icon(onPressed: () => setState(() => learnMode = false), icon: const Icon(Icons.play_arrow_rounded), label: const Text('جرّب بنفسك'))
                  else
                    Expanded(
