...
class GamesScreenV11 extends StatefulWidget {
  ...
  @override
  State<GamesScreenV11> createState() => _GamesScreenV11State();
}
...
class _GamesScreenV11State extends State<GamesScreenV11> {
  ...
  void _answer(String letter) {
    ...
    if (correct) { score += 2; VoiceService.arabic(target.letter); }
  ...
  Widget build(BuildContext context) {
    return Directionality(
      ...
      child: PopScope<Object?>(
        canPop: true,
        onPopInvokedWithResult: (didPop, result) { if (didPop) Future<void>.delayed(Duration.zero, _onExit); },
        child: Scaffold(
  ...
  Widget _hunt() => Column(children: [
    ...
    ...options.map((x) => Padding(padding: const EdgeInsets.only(bottom: 8), child: SizedBox(width: double.infinity, child: FilledButton.tonal(onPressed: () => _answer(x.letter), child: Text(x.letter, style: const TextStyle(fontSize: 30))))),
  ]);
  ...
  Widget _sound() => Card(child: Padding(padding: const EdgeInsets.all(20), child: Column(children: [
    ...
    const SizedBox(height: 10),
    IconButton.filled(onPressed: () => VoiceService.arabic(target.letter), icon: const Icon(Icons.volume_up), iconSize: 40),
    const SizedBox(height: 16),
    ...options.map((x) => ListTile(title: Center(child: Text(x.letter, style: const TextStyle(fontSize: 32))), onTap: () => _answer(x.letter))),
  ])));
  ...
  Widget _match() => Card(child: Padding(padding: const EdgeInsets.all(20), child: Column(children: [
    ...
    const SizedBox(height: 12),
    Wrap(spacing: 10, runSpacing: 10, children: options.map((x) => ChoiceChip(label: Text(x.letter, style: const TextStyle(fontSize: 25)), selected: false, onSelected: (_) => _answer(x.letter))).toList()),
  ])));
}
