    ...
    return Directionality(
      ...
      child: Scaffold(
        ...
        body: ListView(
          ...
          children: [
            Button3D(
              onTap: () => _openGame(context, const _LetterGame()),
            ...
            Button3D(
              onTap: () => _openGame(context, const _NumberGame()),
...
class _LetterGameState extends State<_LetterGame> {
  ...
  void _answer(ArabicLetter chosen) {
    ...
    if (chosen.letter == target.letter) {
      setState(() { locked = true; score++; feedback = _cheers[rnd.nextInt(_cheers.length)]; });
      ProgressV8.addRewards(stars: 1, xp: 5);
      Future.delayed(const Duration(milliseconds: 900), () { if (mounted) setState(_next); });
    } else {
...
class _NumberGameState extends State<_NumberGame> {
  ...
  static String _numberWord(int n) => const {1: 'واحد', 2: 'اثنان', 3: 'ثلاثة', 4: 'أربعة', 5: 'خمسة', 6: 'ستة', 7: 'سبعة', 8: 'ثمانية', 9: 'تسعة', 10: 'عشرة'}[n] ?? '$n';
