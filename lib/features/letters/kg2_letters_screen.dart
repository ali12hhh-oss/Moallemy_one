...
class _Kg2LettersScreenState extends State<Kg2LettersScreen> {
  ...
  void _next(int delta) {
    setState(() => index = (index + delta + arabicLetters.length) % arabicLetters.length);
  }

  void _onFormTap(String form) {
    VoiceService.arabicLetterSound(arabicLetters[index].letter, fallbackText: arabicLetters[index].sound);
  ...
  Widget build(BuildContext context) {
    ...
    return Directionality(
      ...
      child: Scaffold(
        appBar: AppBar(title: Text('الحروف • حرف ${index + 1} من ${arabicLetters.length}')),
        body: Stack(children: [
          Padding(
            padding: const EdgeInsets.all(16),
            child: Column(children: [
              LinearProgressIndicator(value: (index + 1) / arabicLetters.length),
              const SizedBox(height: 14),
              Text('حرف ${letter.sound}', style: const TextStyle(fontSize: 22, fontWeight: FontWeight.w900)),
              const SizedBox(height: 14),
              Row(
                  ...
                  return Expanded(
                    child: Padding(
                      ...
                      child: Button3D(
                        ...
                        child: Column(children: [
                          Text(f.$2, style: const TextStyle(fontSize: 40, fontWeight: FontWeight.bold, color: Colors.white)),
                          const SizedBox(height: 6),
                          Text(f.$1, style: const TextStyle(fontSize: 13, color: Colors.white70)),
                        ]),
                      ),
                    ),
                  );
              ...
              Text('${letter.emoji} ${letter.word}', style: const TextStyle(fontSize: 26, fontWeight: FontWeight.bold)),
              OutlinedButton.icon(
