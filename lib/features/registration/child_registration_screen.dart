...
class _ChildRegistrationScreenState extends State<ChildRegistrationScreen> {
  ...
  bool saving = false;
  static const stages = [
    ('kg1', 'الروضة الأولى', '🎨'), ('kg2', 'الروضة الثانية', '🔤'),
    ('g1', 'الصف الأول', '🌟'), ('g2', 'الصف الثاني', '🚀'), ('g3', 'الصف الثالث', '🏆'),
  ];

  @override void initState() { super.initState(); _load(); }
  Future<void> _load() async {
    final kids = await AppStorage.getChildren(); final active = await AppStorage.activeId();
    Child? c;
    if (active != null) { for (final k in kids) { if (k.id == active) { c = k; break; } } }
  ...
  Future<void> _save() async {
    ...
    if (index >= 0) {
      ...
      final id = DateTime.now().microsecondsSinceEpoch.toString(); kids.add(Child(id: id, name: n, age: _age(stageId), stage: selected)); await AppStorage.setActive(id);
    }
    await AppStorage.saveChildren(kids);
    if (!mounted) return;
    setState(() => saving = false);
    await showDialog<void>(context: context, barrierDismissible: false, builder: (_) => _CelebrationDialog(name: n, stage: selected));
    if (mounted) Navigator.pop(context);
  }
  int _age(String id) => switch (id) { 'kg1' => 4, 'kg2' => 5, 'g1' => 7, 'g2' => 8, _ => 9 };
  void _openPlacementTest() => Navigator.push(context, MaterialPageRoute(builder: (_) => const PrepExamScreen()));
      ...
      child: Scaffold(
        ...
        body: ListView(
          ...
          children: [
            ...
            const SizedBox(height: 22), const Text('اختر مرحلتك الدراسية', style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold)), const SizedBox(height: 12),
            ...stages.map((s) {
              final selected = stageId == s.$1;
              return Padding(padding: const EdgeInsets.only(bottom: 10), child: Button3D(onTap: () => setState(() => stageId = s.$1), color: StageColors.of(s.$1), depth: selected ? 2 : 8, padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 14), child: Row(children: [Text(s.$3, style: const TextStyle(fontSize: 26)), const SizedBox(width: 12), Expanded(child: Text(s.$2, style: const TextStyle(fontSize: 18, fontWeight: FontWeight.w900, color: Colors.white))), if (selected) const Icon(Icons.check_circle_rounded, color: Colors.white, size: 26)])));
            }),
            const SizedBox(height: 6),
            Button3D(onTap: _openPlacementTest, color: const Color(0xFFFFB300), padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 14), child: Row(children: [const Text('📝', style: TextStyle(fontSize: 30)), const SizedBox(width: 12), const Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [const Text('اختبار تحديد المستوى', style: TextStyle(fontSize: 18, fontWeight: FontWeight.w900, color: Colors.white)), SizedBox(height: 4), Text('اختبار للمرحلتين السابقتين وتمهيد للصف الأول — ليس مرحلة دراسية.', style: TextStyle(fontSize: 12.5, color: Colors.white))])), const Icon(Icons.arrow_back_ios_new_rounded, color: Colors.white, size: 18)])),
            const SizedBox(height: 22),
            Button3D(onTap: saving ? () {} : _save, color: const Color(0xFF00C853), padding: const EdgeInsets.symmetric(vertical: 16), child: Row(mainAxisAlignment: MainAxisAlignment.center, children: [const Icon(Icons.save_rounded, color: Colors.white), const SizedBox(width: 10), Text(saving ? 'جارٍ الحفظ...' : 'حفظ والبدء 🚀', style: const TextStyle(fontSize: 18, fontWeight: FontWeight.w900, color: Colors.white))])),
          ],
        ),
      ),
    );
