...
class _ChildRegistrationScreenState extends State<ChildRegistrationScreen> {
  ...
  Future<void> _load() async {
    ...
    if (loaded != null && mounted) {
      setState(() { name.text = loaded.name; stageId = stages.firstWhere((s) => s.$2 == loaded.stage, orElse: () => stages.first).$1; });
    }
  }
      ...
      body: ListView(padding: const EdgeInsets.all(18), children: [
        ...
        ...stages.map((s) {
          ...
          return Padding(padding: const EdgeInsets.only(bottom: 10), child: Button3D(
            onTap: () => setState(() => stageId = s.$1), color: StageColors.of(s.$1), depth: selected ? 2 : 8,
            padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 14),
            child: Row(children: [
              Text(s.$3, style: const TextStyle(fontSize: 26)), const SizedBox(width: 12),
        ...
        Button3D(
          onTap: _openPlacementTest, color: const Color(0xFFFFB300), padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 14),
          child: Row(children: [
        ...
        const SizedBox(height: 22),
        Button3D(
          onTap: saving ? () {} : _save, color: const Color(0xFF00C853), padding: const EdgeInsets.symmetric(vertical: 16),
          child: Row(mainAxisAlignment: MainAxisAlignment.center, children: [
            const Icon(Icons.save_rounded, color: Colors.white), const SizedBox(width: 10),
...
class _CelebrationDialogState extends State<_CelebrationDialog> with SingleTickerProviderStateMixin {
  ...
  @override Widget build(BuildContext context) {
    ...
    return Dialog(backgroundColor: Colors.transparent, child: Container(
      ...
      decoration: BoxDecoration(gradient: const LinearGradient(colors: [Color(0xFFFFD54F), Color(0xFFFF7043)], begin: Alignment.topLeft, end: Alignment.bottomRight), borderRadius: BorderRadius.circular(28)),
      child: Column(mainAxisSize: MainAxisSize.min, children: [
        ScaleTransition(scale: bounce, child: const Text('🎉⭐🎊', style: TextStyle(fontSize: 46))), const SizedBox(height: 14),
        Text(cheer, style: const TextStyle(fontSize: 24, fontWeight: FontWeight.w900, color: Colors.white), textAlign: TextAlign.center), const SizedBox(height: 10),
        Text('تم حفظ اسم ${widget.name} في ${widget.stage} 🌟\nسنحفظ تقدمك مع كل نشاط.', style: const TextStyle(color: Colors.white, fontSize: 15), textAlign: TextAlign.center), const SizedBox(height: 22),
        Button3D(onTap: () => Navigator.pop(context), color: Colors.white, padding: const EdgeInsets.symmetric(vertical: 14), child: const Center(child: Text('هيا نبدأ!', style: TextStyle(fontSize: 17, fontWeight: FontWeight.w900, color: Color(0xFFFF7043))))),
      ]),
    ));
  }
}
