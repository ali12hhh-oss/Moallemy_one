...
class _ChildRegistrationScreenState extends State<ChildRegistrationScreen> {
  ...
  static const stages = <(String, String, String)>[
    ('kg1', 'الروضة الأولى', '🎨'),
    ('kg2', 'الروضة الثانية', '🔤'),
    ('g1', 'الصف الأول', '🌟'),
    ('g2', 'الصف الثاني', '🚀'),
  ...
  Future<void> _load() async {
    ...
    final loadedChild = child;

    if (loadedChild != null && mounted) {
      final loadedStage = stages.firstWhere(
        (stage) => stage.$2 == loadedChild.stage,
        orElse: () => stages.first,
      );
      setState(() {
        name.text = loadedChild.name;
        stageId = loadedStage.$1;
      });
    }
  }
